...
#include <unistd.h>
#include <string.h>
#include "../../Utils/Network/prih/Transmission.h"
...
int loginGuide(int socket){
	...
	while(i < 3){
		...
		if(0 == strcmp("TRUE",unpacketTLVContainer[2])){								//判断TLV的value是否为TRUE，是则表明用户名正确
			//密码校验
			printf("请输入密码\n");
			...
			recvMessage(socket,recvTLVValueContainer,MSG_WAITALL);						//阻塞等待服务器返回的消息
			unpacketTLV(recvTLVValueContainer,unpacketTLVContainer);					//解封TLV数据
			if(0 == strcmp("TRUE",unpacketTLVContainer[2])){							//判断TLV的value是否为TRUE，是则表明密码正确
				return 0;
			}else{
				printf("密码错误！\n");
				continue;
...
*************************************/
int commitCmd(int socket,char* cmd_container){
